...
12-16	-17	Change DNI to add "SPK-" in front of MAC.  This will then
		allow the SuperLANConnect detected device rendition to
		co-exist with this version.  ALLOWS use of Text-to-Speech
		SmartAps with the speakers.
*/
...
def ssdpHandler(evt) {
	...
	if (speakers."${uuid}") {
		...
		def speaker = [:]
		speaker["dni"] = "SPK-${parsedEvent.mac}"
