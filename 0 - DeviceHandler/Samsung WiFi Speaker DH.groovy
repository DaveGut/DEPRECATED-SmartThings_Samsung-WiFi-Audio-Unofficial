...
//	----- GROUP PRESET FUNCTION TILES -----
		...
		standardTile('grpSpkToggle', 'selSpkName', decoration: 'flat', width: 4, height: 1) {
			state 'inactive', label: '-'
 			state 'Toggle Group', label:'${currentValue}', action: 'toggleGroupSpk'
...
//	17-11-24	Changed Soundbar scale to 30.
    if (state.hwtype == "SPK") {
		state.volScale = 30
   ...
   def submode = state.subMode
	switch(submode) {
		...
		default:
            ...
            sendErrorMsg("playPause: Playback Control not valid for device or mode")
		 	return
	}
   ...
   def cp = state.currentCp
    if (submode == "cp") {
    	if (cp != "Amazon" && cp != "AmazonPrime") {
            ...
            sendErrorMsg("previousTrack: Previous Track does not work for this player")
            return
        }
    }
...
def nextTrack() {
    ...
    if (submode == "cp") {
    	if (cp != "Amazon" && cp != "AmazonPrime" && cp != "Pandora" && cp != "8tracks") {
            ...
            sendErrorMsg("nextTrack: Next Track does not work for this player")
            return
        }
    }
...
def schedSetTrackDescription(playtime) {
    ...
    if (nextUpdate <= 3) {
    	log.error "${device.label}_schedSetTrackDescription: Next update value (${nextUpdate}) was negative in schedSetTrackDescription."
...
def rxDataFromSM(speakerData) {
    ...
    if (spksInGroup-1 == subSpkNo) {
	    log.info "${device.label}_rxDataFromSM:  ${preset} created."
...
//	----- PLAY PRESET METHODS
		...
		case "GroupName":
        	...
        	break
        case "MainInfo":
			if (respData.groupmainmacaddr != state.mainSpkMAC) {
				log.error "${device.label}_generalResponse_${respMethod}: MAC Mismatch."	
				setErrorMsg("generalResponse_${respMethod}: MAC Mismatch.")
				return
			}
...
//	#################################################
        ...
        case "ManualSpeakerUpgrade":
 		...
 		default:
        	nextMsg()
	        log.info "${device.label}_generalResponse_${respMethod}: Data: ${respData}"
            break
	}
}
...
def parse(String description) {
	...
	switch(respMethod) {
        ...
        case "7bandEQMode":
        case "MuteStatus":
			log.info "FORWARDED FROM parse, method: ${respMethod}."
			generalResponse(resp)
	        break
        default:
			log.info "${device.label}_parse_${respMethod}. IgnoredMethod."
			break
    }
}
...
def nextMsgResponse(resp) {
	def response = new XmlSlurper().parseText(resp.body)
    def respMethod = response.method
	log.info "${device.label}_nextMsgResponse_${respMethod}:  Forwrded method."
	generalResponse(resp)
}
